...
export default function UsersPage() {
  ...
  async function handleActivate(u: StaffUser) {
    if (!confirm(t('users.activateConfirm', { name: u.name }))) return;
    await apiFetch(`/users/${u.id}`, { method: 'PUT', body: { is_active: true } });
    setReloadKey((k) => k + 1);
  }
  ...
  async function handleDelete(u: StaffUser) {
    ...
    try {
      await apiFetch(`/users/${u.id}?force=1`, { method: 'DELETE' });
      setReloadKey((k) => k + 1);
    } catch (err) {
