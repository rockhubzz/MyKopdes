...
export default function OwnerDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-koperasi-800">Shop Owner Dashboard</h1>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Revenue Today" value={summary ? formatRp(summary.today.revenue) : '...'} />
        <StatCard label="Profit Margin (Month)" value={summary ? `${summary.month_to_date.profit_margin_pct}%` : '...'} accent="harvest" />
        <StatCard label="Stock Value (Retail)" value={summary ? formatRp(summary.stock.total_value_at_retail) : '...'} />
        <StatCard
          ...
          {summary?.best_sellers.map((b) => (
            <li key={b.name} className="flex justify-between border-b border-koperasi-50 pb-1">
              ...
              <span className="text-koperasi-500">
                {b.units_sold} units · {formatRp(b.revenue)}
              </span>
            </li>
          ))}
